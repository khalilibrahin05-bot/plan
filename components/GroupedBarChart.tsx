...
const GroupedBarChart: React.FC<GroupedBarChartProps> = ({ data, title }) => {
  ...
  return (
    <div className="p-4 border rounded-lg bg-white shadow-sm h-full flex flex-col">
        ...
        {data.map((item, index) => (
          <div key={index} className="flex flex-col items-center flex-1 min-w-0 h-full">
            <div className="relative w-full h-full flex items-end justify-center gap-1">
              {/* Bar 1: Planned */}
              <div
                className="w-1/2 bg-green-300 hover:bg-green-400 transition-all duration-300 rounded-t-sm"
                style={{ height: `${(item.value1 / maxValue) * 100}%` }}
                title={`المخطط: ${item.value1}`}
              >
                 <span className="absolute -top-5 left-1/4 -translate-x-1/2 text-xs font-bold text-gray-600">
                  {item.value1}
                </span>
              </div>
              {/* Bar 2: Executed */}
              <div
                className="w-1/2 bg-orange-400 hover:bg-orange-500 transition-all duration-300 rounded-t-sm"
                style={{ height: `${(item.value2 / maxValue) * 100}%` }}
                title={`المنفذ: ${item.value2}`}
              >
                 <span className="absolute -top-5 right-1/4 -translate-x-1/2 text-xs font-bold text-gray-600">
                  {item.value2}
                </span>
              </div>
            </div>
      ...
      <div className="flex justify-center items-center space-x-4 space-x-reverse mt-4 text-sm">
        <div className="flex items-center">
            <span className="w-4 h-4 bg-green-300 rounded-sm ml-2"></span>
            <span>المخطط</span>
        </div>
        <div className="flex items-center">
            <span className="w-4 h-4 bg-orange-400 rounded-sm ml-2"></span>
            <span>المنفذ</span>
        </div>
      </div>
    </div>
