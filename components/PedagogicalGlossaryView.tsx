...
import { pedagogicalTermsData, TermDomain } from '../pedagogicalTerms';
import { PrintIcon, SearchIcon, BookOpenIcon } from './Icons';

const PedagogicalGlossaryView: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredData = useMemo(() => {
    if (!searchTerm.trim()) {
      return pedagogicalTermsData;
    }
    ...
    pedagogicalTermsData.forEach(domain => {
      ...
      if (domain.domain.toLowerCase().includes(lowercasedQuery) || matchingTerms.length > 0) {
        result.push({
          ...domain,
          // If domain matches, show all its terms. If not, show only matching terms.
      ...
      {filteredData.length === 0 ? (
        ...
        <div className="space-y-8">
            ...
            <section key={domain.domain} className="break-inside-avoid">
                <h3 className="text-xl font-bold text-blue-800 bg-blue-100 p-3 rounded-t-lg border-b-2 border-blue-300">
                {domain.domain}
                </h3>
...
export default PedagogicalGlossaryView;